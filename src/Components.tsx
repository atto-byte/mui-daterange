...
function getBackground(props) {
  if (props.isSelected)
    return fromTheme(props.theme, 'selected', props.theme.palette.primary.main);
  if (props.isHighlighted)
    return fromTheme(props.theme, 'highlighted', '#ebebeb');
  if (props.isInRange)
    return fromTheme(props.theme, 'inRange', props.theme.palette.primary.light);
  return '#fff';
}
...
export const SelectableCell = withTheme(
  styled(Cell)({
    borderRadius: props => getBorderRadius(props),
    borderColor: ({ theme, isCurrent }) =>
      isCurrent ? fromTheme(theme, 'current', '#9e9e9e') : 'transparent',
